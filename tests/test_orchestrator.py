...
def test_extract_question_and_resources(orchestrator):
    ...
    """
    question, csv_url, submit_url = orchestrator.extract_question_and_resources(html, "http://example.com")
...
async def test_handle_task_successful(orchestrator):
    with patch('app.orchestrator.render_page_with_retries') as mock_render, \
        ...
        result = await orchestrator.handle_task("http://example.com", "test@example.com", "secret")

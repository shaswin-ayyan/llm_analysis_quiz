...
def test_solve_invalid_secret(client):
    response = client.post("/solve", json={"email": "test@example.com", "url": "http://example.com", "secret": "invalid"})
...
def test_solve_valid_secret(mock_handle_task, client):
    ...
    response = client.post("/solve", json={"email": "test@example.com", "url": "http://example.com", "secret": "test_secret"})

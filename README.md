...
-   `QUIZ_SECRET` (Optional): Secret for securing the `/solve` endpoint.
-   `AIPIPE_PROXY_URL` (Optional): Proxy URL if required for scraping.

### Cloud Deployment (e.g., Railway, Render, GCP)

1.  **Push to GitHub**: Ensure your repository is up to date.
2.  **Connect Repo**: Connect your GitHub repository to your cloud provider.
3.  **Configure Build**:
    -   **Build Command**: `docker build -t app .` (or let the platform auto-detect Dockerfile).
    -   **Start Command**: `docker run ...` (usually handled by the platform).
4.  **Set Variables**: Add your `GEMINI_API_KEY` and other secrets in the platform's environment variable settings.
5.  **Deploy**: Trigger the deployment. The app exposes port `8000`.
